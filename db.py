...
def inventory(uname):
	cursor.execute("SELECT Item.item_name, Item.description, Item.quantity, Item.listed_price, Item.exp_date From Item Join soldAt on soldAt.item_id = Item.item_id AND manages JOIN soldAt ON soldAt.store_id=manages.address_id WHERE manages.username=%s",uname)
	info =  tuplesToList(cursor.fetchall())
	return info
