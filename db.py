...
def popItem(itemName):
	cursor.execute("SELECT selectItem.quantity, Item.item_name, Item.description, Item.exp_date, Item.listed_price, Item.quantity, Item.item_id FROM selectItem JOIN Item ON Item.item_id=selectItem.item_id WHERE Item.food_group=%s",itemName)
